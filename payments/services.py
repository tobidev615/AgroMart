...
@transaction.atomic
def wallet_deposit(user, amount: Decimal, reference: str = "", metadata: dict | None = None) -> WalletTransaction:
    if amount <= 0:
...
@transaction.atomic
def pay_order_from_wallet(user, order: Order, amount: Decimal | None = None) -> Payment:
